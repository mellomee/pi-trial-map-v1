              ...
              <div className="flex items-start gap-3 p-4">
                ...
                <div className="flex items-center gap-1 flex-shrink-0">
                  <Link to={`${createPageUrl("VideoClipEditor")}?id=${clip.id}`}
                    ...
                    <Film className="w-3 h-3" /> Link Video
                  </Link>
                  <button
                    onClick={() => { setLinkExhibitClip(clip); setExhibitSearch(""); }}
                    className="flex items-center gap-1 px-2 py-1 rounded bg-amber-600/15 text-amber-400 hover:bg-amber-600/25 text-[10px]">
                    <BookOpen className="w-3 h-3" />
                    {clip.linked_joint_exhibit_id
                      ? jointExhibits.find(j => j.id === clip.linked_joint_exhibit_id)?.marked_no || "Exhibit"
                      : "Link Exhibit"}
                  </button>
