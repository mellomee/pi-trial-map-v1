  ...
  return (
    <div className="absolute inset-0 pointer-events-none">
      {pageAnns.map(a => {
        ...
        if (!isActive) return null; // show ONLY selected annotation
            ...
            <React.Fragment key={a.id}>
              <svg style={{ position: "absolute", inset: 0, width: "100%", height: "100%", pointerEvents: "none" }}
                ...
                <line x1={g.from?.x} y1={g.from?.y} x2={g.to?.x} y2={g.to?.y}
                  ...
                  markerEnd={`url(#arrowhead-pres-${a.id})`} />
              </svg>
              {g.textBox && (
                <div style={{
                  position: "absolute",
...
export default function Present() {
  ...
  // Load all admitted exhibits + extracts for the case
  useEffect(() => {
  ...
  // Load jury-safe annotations when exhibit changes
  ...
  useEffect(() => {
    ...
    base44.entities.ExhibitAnnotations.filter({ extract_id: extractForExhibit.id })
      .then(anns => {
        // Only JurySafe: jury_safe=true OR in a JurySafe group
  ...
  // When exhibit changes reset page/annotation
  useEffect(() => {
    if (!initAnnotationId) {
  ...
  // Filter annotations to current page
  const pageAnnotations = useMemo(() =>
    annotations.filter(a => (a.page_number ?? a.extract_page_number) === currentPage),
  ...
  return (
    <div className={`flex h-screen bg-black text-slate-100 overflow-hidden ${fullscreen ? "fixed inset-0 z-50" : ""}`}>
                  ...
                  <div className="flex items-start gap-2">
                    ...
                    <div>
                      <p className="text-xs text-slate-200 leading-snug line-clamp-2">
                        {ex.marked_title || ex.internal_name || "—"}
                      </p>
                      {ex.admitted_date && (
                        <p className="text-[10px] text-slate-500">Admitted {ex.admitted_date}</p>
                      )}
                    </div>
                  </div>
                </button>
      ...
      <div className="flex-1 flex flex-col min-w-0">
        ...
        {/* Top Bar */}
        <div className="no-print flex items-center justify-between px-4 py-2 bg-[#0a0f1e] border-b border-[#1e2a45] flex-shrink-0">
          ...
          <div className="flex items-center gap-1.5">
            ...
            <button onClick={() => setScale(s => Math.max(0.5, s - 0.25))} className="p-1 text-slate-400 hover:text-white">
              <ZoomOut className="w-4 h-4" />
            </button>
            <span className="text-[10px] text-slate-600 w-9 text-center">{Math.round(scale * 100)}%</span>
            <button onClick={() => setScale(s => Math.min(3, s + 0.25))} className="p-1 text-slate-400 hover:text-white">
            ...
            <button onClick={() => window.print()} className="p-1 text-slate-400 hover:text-white" title="Print">
        ...
        {/* Center: Document + Right annotation panel */}
          ...
          <div className="flex-1 overflow-auto flex justify-center items-start bg-[#050809] p-6">
            ...
            {fileUrl && isPdf(fileUrl) && (
              <div className="relative inline-block">
                ...
                {/* Print exhibit number watermark */}
                <div className="print-only absolute top-2 right-2 bg-black text-white text-xs font-bold px-2 py-1 rounded">
                  Exh. {displayNumber} — p.{currentPage}
                </div>
              </div>
            )}
                      ...
                      <div key={pg}>
                        <div className="flex items-center gap-1 px-1 py-0.5">
                          ...
                          <button
                            ...
                            className={`text-[9px] px-1.5 rounded transition-colors ${
                              currentPage === pg ? "text-green-400 font-bold" : "text-slate-600 hover:text-slate-400"
                            }`}
                          >
                            p.{pg}
                          </button>
                          ...
                          return (
                            <button key={a.id}
                              ...
                              <div className="min-w-0">
                                ...
                                {!a.label_text && (
                                  <p className={`text-[10px] leading-tight italic ${isActive ? "text-green-400" : "text-slate-500"}`}>
                                    {a.kind} on p.{pg}
                                  </p>
                                )}
                              </div>
