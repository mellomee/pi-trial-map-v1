...
function VideoPlayer({ videoLinks, videoClips }) {
  ...
  useEffect(() => {
    ...
    if (currentTime >= currentLink.end_time_seconds) { v.pause(); setPlaying(false); }
    ...
    <div className="flex flex-col h-full bg-black">
          ...
          {sortedLinks.map((link, idx) => {
            ...
            return (
              <button key={link.id} onClick={() => { setCurrentLinkIdx(idx); setPlaying(false); }}
                className={`px-2 py-0.5 rounded text-[10px] font-medium transition-colors ${idx === currentLinkIdx ? "bg-cyan-600 text-white" : "bg-[#1e2a45] text-slate-400 hover:text-white"}`}>
      ...
      <div className="px-4 py-2 space-y-1.5 bg-black/80 border-t border-[#1e2a45]">
        <div className="flex items-center gap-2">
          ...
          />
          <span className="text-[10px] font-mono text-slate-500 w-12 text-right">{fmt(currentLink?.end_time_seconds ?? duration)}</span>
        </div>
        <div className="flex items-center gap-2 justify-center">
          ...
          <button onClick={stop} className="p-1.5 rounded bg-[#1e2a45] text-slate-400 hover:text-white"><Square className="w-3.5 h-3.5" /></button>
          ...
          {currentLink?.start_time_seconds != null && (
            <button onClick={playSegment} className="px-2 py-1.5 rounded bg-violet-600/30 text-violet-400 hover:bg-violet-600/50 text-[10px]">▶ Segment</button>
          )}
          <button onClick={() => jump(5)} className="px-2 py-1 rounded bg-[#1e2a45] text-slate-400 hover:text-white text-[10px]">+5s</button>
          <button onClick={() => jump(10)} className="px-2 py-1 rounded bg-[#1e2a45] text-slate-400 hover:text-white text-[10px]">+10s</button>
          <button onClick={() => videoRef.current?.requestFullscreen?.()} className="p-1.5 rounded bg-[#1e2a45] text-slate-400 hover:text-white ml-1">
          ...
          {items.map((item, idx) => {
            ...
            return (
              <button key={item.id} onClick={() => onSelect(idx)}
                className={`w-full text-left px-3 py-2.5 transition-colors ${isActive ? "bg-cyan-600/15 border-l-2 border-cyan-500" : "hover:bg-white/5 border-l-2 border-transparent"}`}>
                <div className="flex items-start gap-2">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    {meta.depoName && <p className="text-[10px] text-slate-600 truncate mt-0.5">{meta.depoName}</p>}
...
export default function PresentationMode() {
  ...
  const [exhibitMap, setExhibitMap] = useState({}); // depo_clip_id → JointExhibit
  ...
  const next = () => { markPlayed(); setCurrentIdx(i => Math.min(items.length - 1, i + 1)); };
        ...
        <div className="flex items-center gap-3">
          ...
          <button onClick={markPlayed}
            className={`flex items-center gap-1 px-2 py-1 rounded text-xs transition-colors ${
              currentItem && playedSet.has(currentItem?.id) ? "bg-green-600/20 text-green-400" : "bg-[#1e2a45] text-slate-300 hover:text-white"
            }`}>
