                  ...
                  <div className="flex flex-wrap gap-2 mt-1 items-center">
                    ...
                    {fileInfo && (
                      <button onClick={e => { e.stopPropagation(); setViewFile({ url: fileInfo.url, title: ex.extract_title_official }); }}
                        ...
                        <ExternalLink className="w-3 h-3" /> {fileInfo.isUpload ? "View Extract File" : "View Original File"}
                      </button>
                    )}
                    {/* If there's an uploaded extract file AND an original, also show original */}
                    {ex.extract_file_url && (() => {
                      const primaryId = ex.primary_depo_exhibit_id || ex.source_depo_exhibit_id;
                      const de = primaryId ? depoById[primaryId] : null;
                      const origUrl = de?.file_url || de?.external_link;
                      return origUrl ? (
                        <button onClick={e => { e.stopPropagation(); setViewFile({ url: origUrl, title: `[Original] ${ex.extract_title_official}` }); }}
                          className="text-[10px] flex items-center gap-0.5 hover:underline text-slate-500">
