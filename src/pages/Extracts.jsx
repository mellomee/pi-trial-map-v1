              ...
              <div className="p-4 flex items-start gap-3">
                ...
                <div className="flex-1 min-w-0 cursor-pointer" onClick={() => setExpandedId(isExpanded ? null : ex.id)}>
                  ...
                  <div className="flex flex-wrap gap-2 mt-1 items-center">
                    {/* Source info */}
                    {groupIds.length > 1 ? (() => {
                      const firstSourceDepo = depoById[groupIds[0]];
                      const srcGroupName = firstSourceDepo?.group_name;
                      return (
                        <span className="flex items-center gap-1 text-[10px] text-slate-500">
                          <Users className="w-3 h-3 text-indigo-400" />
                          {srcGroupName ? <span className="text-indigo-300">{srcGroupName}</span> : null}
                          <span className="text-slate-600">({groupIds.length} exhibits)</span>
                          {primaryDepo && <span className="text-slate-600">· primary: <span className="text-cyan-400/70">{depoLabel(primaryDepo.id)}</span></span>}
                        </span>
                      );
                    })() : primaryDepo ? (
